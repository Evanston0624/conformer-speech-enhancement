...
def wav2ibm(WavData, WinLen=512, NUM_FFT = 512, hop_length=200, NUM_FRAME = 1, returnPhase = False):
    D = librosa.stft(y = np.float32(WavData),
                     n_fft = NUM_FFT,
                     hop_length = hop_length,
                     win_length = WinLen,
...
def ibm2wav(Inp_1, Inp_2, WinLen = 512, NUM_FFT = 512, hop_length = 200):
    InpSpec = np.multiply(np.array(Inp_1), np.exp(1j * Inp_2))
    return np.real(librosa.istft(InpSpec,
                             hop_length=hop_length,
                             win_length=WinLen,
                             window=ssg.windows.hann))